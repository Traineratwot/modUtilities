...
- **getUserPhoto** почти полная копия стандартной функции $modx->user->getPhoto() но с 2 существенными плюсами
    ...
    * @return string 
    */
	echo $modx->getUserPhoto($id = 0, $alt = FALSE, $width = 128, $height = 128,$r='g', $default = '404') //аватар текущего пользователя
	echo $modx->getUserPhoto(2,'/path/no_ava.png') // заглушка так как у пользователя 2 нет аватара
    ```
    ```fenom
    //fenom
