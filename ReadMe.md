 ...
 - **Csv** 
   ...
   **запись**
    ...
    **чтение**
    ...
    ```
 - **Rest** 
    позволяет быстро и безопастно настроить rest для вашего сайта
    особенности:
   - полностью кастомные url
   - работает как со снипетами так и с файлами
   - гибкая настройка безопасности
   - basci auth, в качестве пользователя использует modUser
   - поддержка почти всех реез методов
   - интерфейс в админке
   - 1000 полследних подключений
