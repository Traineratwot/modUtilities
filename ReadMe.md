...
- **mb_ucfirst** - возвращает и строку с заглавной буквы для любой кодировки
    ...
    третий параметр $otherLower - если true то приводит всю строку к нижнему регистру перед работой(по умолчанию true)
...
- **ping** ghjdthztn url на доступностть (не возврощает время ответа)
    использует функцию fsockopen с таймоутом 10ms, в случаи если эта функция отключена проверяет заголовки
    ```php
        $modx->util->ping($url); // true
    ```
