...
var self=this
MODx.msg.confirm({title:_('delete'),text:_('confirm_remove'),url:modUtilConnector_url,params:{action:'mgr/rest/rest/delete',id:cs,},listeners:{'success':{fn:function(r){if(!r.success){MODx.msg.status({title:_('undeleted'),message:'Ошибка',delay:3})}else{MODx.msg.status({title:_('delete'),message:'Готово',delay:3})}
...
return m},del:function(){var cs=this.getSelectedAsList()
var self=this
MODx.msg.confirm({title:_('delete'),text:_('confirm_remove'),url:modUtilConnector_url,params:{action:'mgr/rest/category/delete',id:cs,},listeners:{'success':{fn:function(r){if(!r.success){MODx.msg.status({title:_('undeleted'),message:'Ошибка',delay:3})}else{MODx.msg.status({title:_('delete'),message:'Готово',delay:3})}
self.refresh()},scope:true}}})}}]},{title:'log',id:'modx-tabs-log',items:[{html:'connection log',cls:'panel-desc',},{id:'log-main-table',xtype:'modUtil-grid',columns:[{dataIndex:'id',width:330,sortable:true,header:'id',renderer:defaultRenderer,editor:{xtype:'textarea'},},{dataIndex:'rest_id',width:330,sortable:true,header:_('modutilities.rest_id'),renderer:defaultRenderer,editor:{xtype:'textfield'}},{dataIndex:'input',width:670,header:_('modutilities.input'),renderer:JSONRenderer,editor:{xtype:'textarea'},},{dataIndex:'output',width:670,header:_('modutilities.output'),renderer:HTMLRenderer,editor:{xtype:'textarea'},},{dataIndex:'user',sortable:true,width:670,header:_('modutilities.user'),renderer:JSONRenderer,editor:{xtype:'textarea'},},{dataIndex:'time',sortable:true,width:670,header:_('modutilities.time'),renderer:defaultRenderer,editor:{xtype:'textarea'},},{dataIndex:'datetime',sortable:true,width:670,header:_('modutilities.datetime'),renderer:defaultRenderer,editor:{xtype:'textarea'},},],fields:['id','rest_id','input','output','user','time','datetime',],tbar:[{emptyText:_('search'),id:'search-field-log',xtype:'textfield',},{id:'search-button-log',xtype:'button',text:'🔎',cls:'primary-button',handler:function(){var v=Ext.getCmp('search-field-log').getValue()
...
Ext.applyIf(config,{title:'Create thing',width:window.innerWidth/100*50,saveBtnText:'Сохранить 💾',url:modUtilConnector_url,})
...
return true},success:function(){MODx.msg.status({title:_('created'),message:'Готово',delay:3})
...
//# sourceMappingURL=snippet_rest.tab.min.js.map
